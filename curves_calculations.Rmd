...
$$grp = 100 \times \frac{impression\_target}  {audience\_size}$$
With `reaches` it's similar but as there are several of them we will go with:
`... %>% mutate(across(starts_with("reach"), ~{100 * ./audience_size})) %>% ...`
...
***
**One important note:**
Not applying any constraints on $\beta$ might result in better fit but then we loose convenient interpretability (as the asymptotic reach (?)).
